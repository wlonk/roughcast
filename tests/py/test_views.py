...
class TestUserViewSet:
    ...
    def test_profile__put__bad(self, client):
        data = {}
        response = client.put("/api/user/profile/", data, format="json")
        assert response.status_code == 400
    ...
    def test_verify_email__invalid(self, client):
        response = client.post("/api/user/verify_email/")
        assert response.status_code == 400

    def test_verify_email__error(self, client):
        data = {"key": "bad key"}
        response = client.post("/api/user/verify_email/", data)
        assert response.status_code == 400
    ...
    def test_reset_password__invalid(self, client):
        response = client.post("/api/user/reset_password/")
        assert response.status_code == 400

    def test_reset_password__good(self, client):
        data = {"email": "test@example.com"}
        response = client.post("/api/user/reset_password/", data)
        assert response.status_code == 202

    def test_reset_password_confirm__invalid(self, client):
        response = client.post("/api/user/reset_password_confirm/")
        assert response.status_code == 400
    ...
    def test_reset_password_confirm__good(self, client):
        ...
        response = client.post("/api/user/reset_password_confirm/", data)
        assert response.status_code == 204
...
class TestVersionViewSet:
    def test_archive_link(self, client, version, attached_file_factory):
        ...
        response = client.get(f"/api/version/{version.id}/archive/")
